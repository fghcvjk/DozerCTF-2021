...
def rsaEncrypt(str):
    (pubkey, privkey) = rsa.newkeys(340)
    f = open('pk.txt','a')
    print(pubkey,end='',file = f)
    f.close()
...
def test():
    with open('key.txt','r') as f:
        key=f.read()
    key128 = bytes().fromhex(key)
    
    string=s_b
    step=32
    string1=[string[i:i+step] for i in range(0,len(string),step)]
    if len(string1[len(string1)-1])<32:
      string1[len(string1)-1]=string1[len(string1)-1].ljust(32,'0')

    str, pk = rsaEncrypt(key)
    str1=binascii.hexlify(str)
    f = open('keyen.txt','a')
    print(bytesToString(str1),end='',file = f)
    f.close()
    ...
    for KEYS in string1:
        ...
        f = open('encrypto.txt','a')
        print(bytesToString(cipher),end='',file = f)
        f.close()
